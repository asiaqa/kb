...
#tor &
ssh-keygen -A
cat /etc/caddy/Caddyfile | sed -e "s/\$FRP_USER/$FRP_USER/g" -e "s/\$MYPATH/$MYPATH/g" -e "1c :$PORT" -e "s/\$AUUID/$AUUID/g" -e "s/\$FRP_PASS-HASH/$(caddy hash-password --plaintext $FRP_PASS)/g" >/etc/caddy/Caddyfile
cp /x.json /temp.json
